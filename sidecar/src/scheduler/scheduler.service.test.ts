...
describe('SchedulerService', () => {
  ...
  test('add creates job in DB and registers with QueueService', async () => {
    await service.add({
      name: 'daily-report',
      expr: '0 9 * * *',
    ...
    expect(mockQueueService.addJob.mock.calls[0][0]).toEqual({
      type: 'cron',
      name: 'daily-report',
      expression: '0 9 * * *',
      pattern: 'scheduler.fire.daily-report',
    });
  });

  test('add with existing job calls updateJob instead of addJob', async () => {
    mockQueueService.hasJob = mock(() => true);
    ...
    expect(mockQueueService.updateJob).toHaveBeenCalledTimes(1);
  });

  test('remove deletes from DB and QueueService', async () => {
    mockQueueService.hasJob = mock(() => true);

    await service.remove('daily-report');

    expect(mockRepo.deleteByName).toHaveBeenCalledWith('daily-report');
    expect(mockQueueService.removeJob).toHaveBeenCalledWith('daily-report');
  });
  ...
  test('list enriches DB jobs with runtime info', async () => {
    ...
    mockQueueService.getJob = mock(() => ({ nextRun: new Date('2026-03-20T09:00:00Z'), isRunning: false }));
